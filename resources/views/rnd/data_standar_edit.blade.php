          ...
          <div class="card-body">
						<div class="card-header flat bg-secondary text-center">
							@foreach($standar as $s)
								...
								<h5 class="">Edit WIP</h5>
								@else
								<h5 class="">Edit Bahan Baku</h5>
								@endif
							@endforeach
						</div>
						<table class="table table-bordered text-center">
          		@foreach($standar as $s)
							<form class='form' method='post' action="/rnd/data/standar/update">
							{{ csrf_field() }}
									...
									<select class="form-control select2" name="sub_kategori">
										@foreach($standar as $s)
											@foreach($tb_sub_kategori as $tb)
												...
												<option value="{{$tb->id_sub_kategori}}" disabled>{{$tb->nama_kategori}}</option>
												@else
									...
									<select name="plant" required class="form-control" tooltip="Plant">
										@foreach($standar as $s)
											@foreach($tb_plant as $tb)
												...
												<option value="{{$tb->id_plant}}" selected="selected">{{$tb->plant}}</option>
												@else
													@if($tb->status == 'non-aktif')
													<option value="{{$tb->id_plant}}" disabled>{{$tb->plant}}</option>
													@else
							...
							<tr>
								...
								<td><input name='catatan_serving_size' type="text" class='form-control' value='{{$s->catatan_serving_size}}'></td>
							</tr>
							<tr>
								<td>Stok</td>
								<td>
									<div class="row">
										<div class="col-sm-5">   
											<div class="input-group input-group-xs">
												<input  name='stok_rnd' type="number" class='form-control' value='{{$s->stok_rnd}}'>
										...
										<div class="col-sm-5">
											<div class="input-group input-group-xs">
												<input readonly class='form-control' value='<?php echo ceil($s->stok_rnd / $s->serving_size)?>'>
												<div class="input-group-append">
